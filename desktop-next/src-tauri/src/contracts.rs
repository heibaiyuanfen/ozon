...
fn wp(text: &str, bold: bool) -> String {
    format!("<w:p><w:pPr><w:spacing w:after=\"60\" w:line=\"280\" w:lineRule=\"auto\"/></w:pPr><w:r><w:rPr>{}<w:rFonts w:eastAsia=\"宋体\"/></w:rPr><w:t xml:space=\"preserve\">{}</w:t></w:r></w:p>",if bold{"<w:b/>"}else{""},xml(text))
}
fn wc(text: &str, width: i32) -> String {
    format!("<w:tc><w:tcPr><w:tcW w:w=\"{}\" w:type=\"dxa\"/><w:tcBorders><w:top w:val=\"single\" w:sz=\"4\"/><w:left w:val=\"single\" w:sz=\"4\"/><w:bottom w:val=\"single\" w:sz=\"4\"/><w:right w:val=\"single\" w:sz=\"4\"/></w:tcBorders></w:tcPr>{}</w:tc>",width,wp(text,false))
}
fn word_document(v: &ContractInput) -> String {
    ...
    let mut b=String::from("<w:p><w:pPr><w:jc w:val=\"center\"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val=\"36\"/><w:rFonts w:eastAsia=\"黑体\"/></w:rPr><w:t>采购合同</w:t></w:r></w:p>");
    ...
    for x in &v.items {
        b.push_str("<w:tr>");
        for (t, w) in [
            (x.product_name.clone(), 3200),
            (x.unit.clone(), 800),
            (x.quantity.to_string(), 1000),
            (format!("¥ {:.2}", x.unit_price_ex_tax), 1500),
            (format!("¥ {:.2}", x.quantity * x.unit_price_ex_tax), 1500),
        ] {
            b.push_str(&wc(&t, w))
        }
        b.push_str("</w:tr>")
    ...
    b.push_str("</w:tbl>");
    b.push_str(&wp(&format!("合计：以上为不含税价，税率 {}%，税费 ¥ {:.2}，价税合计：{}（¥ {:.2}）；未税金额 ¥ {:.2}。",v.tax_rate,tax,v.amount_uppercase,total,subtotal),false));
    for (h, t) in [
        ...
        b.push_str(&wp(h, true));
        b.push_str(&wp(t, false))
    }
    b.push_str(&wp("注：本合同书一式两份，双方各执一份。", true));
    for t in [
        format!(
            "乙方（公章）：{}　　　　　　　　　甲方（公章）：厦门非凡智汇电子商务有限公司",
            v.seller_name
        ),
        format!(
            "乙方税号：{}　　　　　　　　　甲方税号：91350203MAEB4G4K7T",
            v.seller_tax_no
        ),
        format!(
            "乙方地址：{}　　　　　　　　　甲方地址：厦门市思明区体育路78号801室",
            v.seller_address
        ),
        format!(
            "乙方电话：{}　　　　　　　　　甲方开户行：招商银行股份有限公司厦门体育中心支行",
            v.seller_phone
        ),
        format!(
            "乙方开户行：{}　　　　　　　　甲方账号：592910153310001",
            v.seller_bank
        ),
        format!(
            "乙方账号：{}　　　　　　　　　日期：{}",
            v.seller_account, v.contract_date
        ),
    ] {
        b.push_str(&wp(&t, false))
    }
    format!(
        r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>{}<w:sectPr><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="700" w:right="800" w:bottom="700" w:left="800"/></w:sectPr></w:body></w:document>"#,
        b
    )
}
