...
- 每日产品、产品利润、周报和跨境利润与 Python 旧版逐字段一致性。
- Finance 佣金、退货费用、未分摊费用的 SKU 归属。
- Ozon 商品图片缓存和订单图片展示。
- 库存 API 与 Seller 后台字段的一一对应。
- 广告历史 SKU 明细覆盖和大型图表性能。
- WB 经营总览、订单、广告、库存、利润和同步的完整模块化。
- 竞品采集：直连失败时的浏览器会话/验证页处理。
- 大型 React 主包动态分包，当前 Vite 会提示主包超过 500 kB。

### 尚未完成的 RFBS 上品关键流程

- Ozon 类目树下载、缓存和搜索。
- 旧版 `rank_categories` 类目推荐完整移植。
- 属性定义、组合属性和字典值加载。
- 参考属性到 Ozon 属性的保守匹配。
...
- 导入任务状态轮询、错误明细和断点续跑。
- 成功结果回写产品台账。
- 旧版 Playwright/Edge 验证页采集方案在 Tauri 中的安全实现。
