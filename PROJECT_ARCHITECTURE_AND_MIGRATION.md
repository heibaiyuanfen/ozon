...
## 2026-09-10：补货装箱单与箱唛生成模块

- 在 Ozon「库存与供应链」导航新增「补货装箱」页面，可从当前店铺库存选择多个产品，也支持手工填写商品。
- 页面支持批次号、标题、平台、店铺、出货日期、注意事项，以及每个产品的条码、箱数、单箱数量、重量、尺寸、带电属性和备注。
- 新增本地生成器：按每箱一行生成标准 XLSX 发货装箱单，体积列使用长×宽×高÷1,000,000 公式；按每箱一页生成 7×5 英寸横向 PDF 箱唛，并自动打开导出目录。
- 验证：`pnpm build` 成功；`cargo check --locked` 成功；Rust 全量测试 127 通过、4 个需外部环境的测试忽略；新增 XLSX ZIP 结构和多页 PDF 自动化测试通过。
- 发布：使用 `desktop-next/scripts/build-tauri-release.cmd` 成功构建，Release 产物覆盖根目录 `ozon-analytics-next.exe`。当前产物为 30,260,736 字节，SHA-256 为 `0B7A65ED3A2AA1A895C2B48E88C76E7C57B5A7EDC620E4DD0FB002227B9368A6`。
- 启动验收：根目录程序成功启动并保持响应，主窗口句柄已创建；进程未连接 `localhost:1420`，确认使用内嵌正式前端。当前自动化环境未暴露原生窗口截图接口，因此页面的像素级人工验收留待实际操作时复核。
- Excel 兼容性修复：纠正 worksheet XML 中 `autoFilter` 与 `mergeCells` 的元素顺序，并补齐默认样式和工作表属性。修复前文件可被宽容解析器读取，但 Microsoft Excel 会触发修复；修复后通过严格告警模式重新打开，保留 12 列、数据行和体积公式。
- 草稿缓存：新增 `packing_drafts` 本地表，页面修改后延迟自动保存，并支持草稿命名、立即保存、载入、新建和删除。生成文件前会再次保存当前草稿。
- 飞书归档：连接设置新增“发货装箱单文件夹链接或 Token”，生成后可把 XLSX 和 PDF 通过飞书云盘文件上传接口归档至目标文件夹；支持直接粘贴文件夹链接并自动提取 Token。实际上传仍取决于飞书应用的云盘权限及目标文件夹协作者权限。
- 回归：`pnpm build`、`cargo check --locked` 和 Rust 全量测试通过（128 通过、4 个外部环境测试忽略）；文件夹 Token 解析、XLSX/PDF 生成测试通过。
- 上传按钮反馈修复：最近生成文件路径写入前端持久缓存，重启或返回页面后仍可上传；无文件时点击会明确提示先生成，上传开始、成功和失败均显示状态。飞书文件夹和凭证允许从其他已配置店铺读取，解决切换店铺后配置不可见的问题。
- 飞书 HTTP 400 诊断修复：multipart 的二进制文件头改为纯 ASCII 文件名，真实中文文件名继续通过 `file_name` 字段提交，避免网关因非 ASCII `filename` 返回通用 400；对于非 2xx 响应，保留并展示飞书 JSON 错误码、消息及请求 ID，便于准确区分参数和权限问题。

## 2026-09-10 — Wildberries 价格与活动中心

- 新增 `price_center.rs`，复用 WB 店铺凭据、商品映射、同步任务和审计系统；建立 append-only 价格快照、价格规则、改价任务/明细、RAW 响应与促销日历结构。
- 新增 WB 工作区“价格与活动中心”页面：价格总览、SKU 价格、强制 Preview 的批量改价、安全价规则、活动、历史和任务。
- 金额写入使用十进制字符串解析为分，禁止浮点金额进入领域校验；UNMAPPED/CONFLICT 被阻止，缺成本返回 `insufficient_data`，平台写入成功仅标记 `pending_verify`。
- 仓库审计见 `docs/wb-price-promotion-audit.md`。
- 验证：`cargo check --locked` 通过；Rust tests 130 passed / 4 ignored；`pnpm build` 通过。正式 Release 构建、根目录覆盖、哈希与可见验收记录见本节后续更新。
- 正式交付：最终一次 `desktop-next/scripts/build-tauri-release.cmd` 成功；Release EXE 已覆盖根目录，二者 SHA-256 均为 `F64E1ABC4612B6C0F952A76E07CA6A72D1D2C588706D629FFE592ECF68D84EC5`，大小 `30,319,104` bytes；根目录程序启动后进程正常响应。当前会话没有可用的原生窗口捕获面，未完成页面截图级可见验收。

## 2026-09-10 — 采购单添加模块

- 新增独立于采购合同的采购单中心，入口位于“经营管理 → 采购单添加”。
- 采购单支持新建、草稿保存、历史载入修改、归档删除、打印/PDF；明细覆盖品名、SKU、图片 URL、单价、包装规格、单品重量、装箱率、外箱重量/规格、箱数、数量、单位和备注。
- 页面自动汇总箱数、采购数量、采购总额、外箱总重量和总体积，并保留采购单号、经办人、采购日期、预计出货日期、工厂地址和审批人。
- SQLite 新增 `purchase_orders` 和 `purchase_order_items`；采购单号在未归档数据中唯一，保存使用事务，数值采用最多四位小数的字符串校验，避免表单浮点噪声。
- 验证：`cargo check --locked`、采购单专项测试和 `pnpm build` 均通过；全量 Rust 测试 132 passed / 4 ignored。
- 正式交付：`desktop-next/scripts/build-tauri-release.cmd` 成功；Release EXE 已覆盖根目录，SHA-256 为 `9FD6144053B2407094148322F27D67902566966A585A19C20070DB76370DBEF7`，大小 `30,387,712` bytes；根目录进程启动并正常响应。未执行真实采购单写入，以免在用户正式数据库中制造测试记录。

### Excel 导出补充

- 采购单页面增加“导出 Excel”，输出真正的 `.xlsx` 文件并自动打开。
- 工作簿包含 23 列采购、包装和出货字段；总价、密度、总重、体积及合计均为 Excel 公式，数字保持原生数值类型。
- 设置中文字体、合并标题、重点输入列、冻结前三行、自动筛选及 A4 横向打印布局。
...
- Seller Finance HTTP 错误诊断：Seller API 非 2xx 响应现读取并展示 Ozon 返回正文（压缩空白并限制 800 字符），不再丢失 400 的平台错误码与消息；Finance 逐笔交易与现金流错误补充请求日期区间、页码以及“本地缓存未清除”说明。新增错误正文与空正文测试；`cargo check --locked`、`pnpm build` 通过，Rust 全量测试 135 passed / 4 ignored。正式脚本构建并覆盖根目录 EXE，Release/root SHA-256 均为 `A269AC576C0722F8CC4D29E4A2BC2C59A4F0751145F9A78992AFDFA87AFED2E5`，大小 `30,461,952` bytes；根目录程序已启动且响应正常。
- Seller Finance 时间格式修复：根据平台返回 `code: 9 / timezone method cannot be used`，将逐笔交易与现金流请求时间从无毫秒的 `T00:00:00Z`/`T23:59:59Z` 统一为接口示例使用的 RFC3339 毫秒 UTC 格式 `T00:00:00.000Z`/`T23:59:59.999Z`。新增时间序列化回归测试；`cargo check --locked`、`pnpm build` 通过，Rust 全量测试 136 passed / 4 ignored。正式脚本构建并覆盖根目录 EXE，Release/root SHA-256 均为 `63CA02DFADC78A3D7E5CF32F91C962AC6FB5EC87B7C45EEBEE50031C7F303D19`，大小 `30,461,952` bytes；根目录程序已启动且响应正常。
- Seller Finance 废弃接口迁移：Ozon 已停用 `/v3/finance/transaction/list` 并返回 `obsolete method cannot be used`，财务明细同步迁移至 `/v1/finance/accrual/by-day`，按自然日请求并使用 `last_id` 游标分页；同时读取 `/v1/finance/accrual/types`，新增兼容转换层，将应计销售额、销售佣金、配送费用、SKU、发货单号和总金额映射到既有报表字段，避免破坏利润及商品分析。新增新结构转换测试；`cargo check --locked`、`pnpm build` 通过，Rust 全量测试 137 passed / 4 ignored。正式脚本构建并覆盖根目录 EXE，Release/root SHA-256 均为 `D5E277D4F49CFD7198FD92A549971848FFCE4DC49AED68D8033A28107C943E20`，大小 `30,447,104` bytes；根目录程序已启动且响应正常。尚未使用用户店铺凭证执行真实同步，需由用户在界面重新同步完成最终数据验收。
- Finance 广告应计对账修复：按用户后台账单逐项复核后，确认 `StarsMembership` 属于会员服务而非“推广和广告”，并补充新应计接口服务名 `PayPerClick` 的广告分类；报表缓存指纹升级，历史月份无需清库即可重算。现有数据库回放结果：2026-08 广告应计由 `-1,500,059.92` 修正为 `-1,320,832.46` RUB，与后台取整值 `-1,320,832` 对齐；2026-09 同步快照修正为 `-398,436.73` RUB，与稍后后台截图 `-398,724` 的 `287.27` RUB 差额对应当日新增应计。验证：`cargo check --locked`、`pnpm build` 通过，Rust 全量测试 137 passed / 4 ignored。正式脚本构建并覆盖根目录 EXE，Release/root SHA-256 均为 `3F581E66B9955F2300A87A457AA8F60EDD3066ECE7049D35213CD0A29C80E095`，大小 `30,447,104` bytes；根目录程序已启动且响应正常。当前会话未执行原生窗口截图级验收。
- 广告实验阶段流转修复：`next_stage` 改为幂等操作，同一父阶段/目标阶段只保留一个活动子实验；创建下一阶段后父阶段自动标记 `completed`，默认列表隐藏已完成阶段，仍可通过“已完成”筛选查看。初始化兼容整理会将旧版产生的重复子阶段中较早记录归档，保留最新记录为活动阶段，不删除历史数据；本次已对当前默认本土店的既有 `wzw` 与 `WZW001-Desert Digital-10` 重复阶段执行整理。新增流转幂等回归测试；`cargo check --locked`、`pnpm build` 通过，Rust 全量测试 138 passed / 4 ignored。正式脚本构建并覆盖根目录 EXE，Release/root SHA-256 均为 `EC16BEE45EED00844CC6B893E33BB9690F233C871094568837F967A745CEBDEC`，大小 `30,448,128` bytes；根目录程序已启动且响应正常。当前会话未执行原生窗口截图级验收。

## 2026-09-14 — 广告实验下一阶段门槛调整

- “进入下一阶段”的资格改为仅检查三项业务标准：观察完整日数量达到实验设定值、观察期整体 TACOS 不高于当前阶段目标、观察期日均销量不低于当前阶段目标。
- 评分、置信度、库存覆盖、逐日连续达标、诊断否决和平台执行确认继续展示为风险信息，但不再锁定下一阶段按钮；下一阶段仍只创建待核对草稿，不自动修改 Ozon。
- 现有 `wzw` 实验按已保存评估数据满足 Day 3/3、日均销量 57.33 ≥ 20、TACOS 4.50% ≤ 10%，点击“刷新并评估”后应允许进入下一阶段。未直接改写该实验状态，以保留用户业务记录。
- 验证：广告实验专项测试 23 passed；`cargo check --locked`、`pnpm build` 通过；Rust 全量测试 137 passed / 4 ignored。正式脚本构建并覆盖根目录 EXE，Release/root SHA-256 均为 `B7B9590F31784E54A0569EC9B49498F9AB02C3DF119F60B2D29F05880FC38DD1`，大小 `30,529,024` bytes；根目录程序已启动且响应正常。当前会话未执行原生窗口截图级验收。

## 2026-09-14 — 跨境店铺利润周报发送到飞书群

- 在“报表与利润 → 跨境店铺利润”的日期工具栏新增“发送周报到飞书群”；使用页面当前店铺和日期范围，发送前由后端调用同一跨境利润核算函数重新计算，避免发送旧缓存或本土店铺口径。
- 飞书互动卡片包含店铺、统计期间、销售额、销量、预估利润、Performance/Stars 广告拆分、预估平台费用、采购及头程、Finance 已结算净额、FBP/RFBS/WHD 履约订单及人民币兑卢布汇率；成本缺失时明确显示缺失 SKU 数，不伪造利润。
- 发送复用当前跨境店铺的飞书 App ID、App Secret 与群 Chat ID，并校验当前店铺类型；未配置时返回可操作提示。新增卡片内容回归测试，不在自动化测试中向真实群发送消息。
- 验证：`cargo check --locked`、`pnpm build` 通过；Rust 全量测试 139 passed / 4 ignored，新增跨境飞书卡片测试通过。正式脚本构建并覆盖根目录 EXE，Release/root SHA-256 均为 `FC8FF5DAF5862764ED58079BB6945330B336F64BF2CEA0A83390F4A776C7231A`，大小 `30,561,792` bytes。
- 可见验收：根目录正式程序成功启动，窗口响应正常且加载内嵌前端；跨境利润页面已显示发送按钮及真实核算数据。首次验收发现工具栏 CSS 将按钮渲染成白底白字，已修正为深色可见按钮并重新构建发布。未代替用户向真实飞书群发送消息；当前跨境店铺还提示旧凭证无法由 Windows DPAPI 解密，需在“连接设置”重新保存该店铺凭证后才能完成真实发送。
- 库存发送补充：跨境利润周报新增当前店铺库存快照，汇总 SKU 数、可售、在途、待供货、国内生产/仓库及建议补货量，并逐项列出最多 20 个缺货、7 天内断货或库存偏低 SKU（货号、可售、在途、待供货、日均销量、健康状态、建议补货）。明细上限用于避免飞书互动卡片超长被拒绝。
- 库存补充验证：`cargo check --locked`、`pnpm build` 通过；Rust 全量测试 139 passed / 4 ignored，卡片测试覆盖库存汇总与低库存明细。正式脚本重新构建并覆盖根目录 EXE，Release/root SHA-256 均为 `420CF6C43CE6D20FC1BFF923291C10ACE1895E593D41F7A66A96A5C0948742DD`，大小 `30,552,576` bytes；根目录程序已重新启动并创建 `Ozon ERP` 窗口。未向真实群发送验收消息。

### 库存管理独立发送修正

- 按用户确认后的业务边界，将库存内容从跨境利润周报中移除；利润按钮恢复为只发送利润数据。
- 在“库存与供应链 → 库存管理”顶部新增独立“发送库存与补货到飞书群”按钮，只读取库存管理口径，并沿用页面当前目标库存天数、采购运输天数和安全库存天数重新计算补货建议。
- 飞书库存卡片首屏提供 SKU、可售、Ozon 在途、已申请、国内生产、国内仓、海外在途、海外到仓、建议补货及风险 SKU 汇总；每个 SKU 以分隔符表格行展示货号/SKU、预留、各供应链阶段、日均销量、覆盖天数、建议/计划及健康状态。全部 SKU 每 20 项自动分页发送，不截断为风险商品前 20 项。
- 验证：`cargo check --locked`、`pnpm build` 通过；Rust 全量测试 140 passed / 4 ignored，新增库存与补货字段及独立卡片回归测试通过。正式脚本构建并覆盖根目录 EXE，Release/root SHA-256 均为 `4F5EC2AD0D0B169963F5CBF0AB8A355C524B0CC7C913BDD99265EA11AA198C62`，大小 `30,454,784` bytes；根目录正式程序已启动并保持响应。因用户正在操作窗口，未继续自动导航或向真实飞书群发送验收消息。

### 库存飞书发送范围自选

- 库存明细增加逐项复选框与当前页全选，可按当前风险/搜索筛选批量选择或清空选择；发送按钮实时显示已选 SKU 数，未选择商品时禁止发送。
- 库存页读取已有产品系列，用户选择系列后可一次加入该系列中与当前库存匹配的全部 SKU；没有匹配时显示明确提示，不把不存在的 SKU 计入发送数量。
- 前端将已选 SKU 列表提交给后端，后端再次按精确 SKU 白名单过滤库存计算结果，因此飞书卡片的汇总、库存明细与补货建议都只包含用户选中的商品，避免仅靠界面过滤造成整店误发。
- 验证：`cargo check --locked`、`pnpm build` 通过；Rust 全量测试 140 passed / 4 ignored。正式脚本构建并覆盖根目录 EXE，Release/root SHA-256 均为 `83BBEA1B7A9B24B6F6B191310C1B5A9D9524EDE5AE9B9ED94D0578613205A359`，大小 `30,463,488` bytes。
- 启动验收：根目录正式程序已成功启动并创建唯一 `Ozon ERP` 窗口；准备继续可见页面检查时检测到用户正在操作该窗口，为避免干扰未接管界面，也未向真实飞书群发送测试消息。

## 2026-09-14 — 采购单提交飞书审核

- “采购单添加”页面新增“提交审核到飞书群”按钮；普通“保存草稿”保持本地操作，不会向群内发送消息。提交审核会先校验并保存当前最新表单，再调用当前店铺的飞书应用与群 Chat ID。
- 飞书互动卡片标记为“待审核”，包含采购单号、采购/预计出货日期、经办人、审批人、工厂地址、商品项数、数量、箱数、总金额，以及每项商品的名称、SKU、单价、数量、箱数、装箱率、小计和备注；超过 15 项自动分页。
- 发送成功后采购单状态更新为 `submitted`，历史列表显示“已提交飞书”；已提交采购单再次修改保存后恢复为草稿，要求重新提交，避免领导看到的版本与软件当前版本不一致。发送失败时本地草稿保留并给出重新配置后再提交的提示。
...
## 2026-09-18 全店自动同步固定最近七天

- 软件启动自动同步与周期定时同步统一固定为“含今天在内最近 7 个自然日”，不再允许选择 14、30、60 或 90 天，减少每轮 Seller、Performance 与 Finance 请求时间。
- 后端使用固定常量生成日期范围，不信任客户端或旧配置中的回溯天数；历史 `auto-sync.json` 即使保存了更长周期，读取时也会自动归一为 7 天。
- 数据同步页面移除可变回溯范围下拉框，改为“最近 7 天（固定）”状态，启动同步和定时同步开关均明确标注七天范围；手工同步日期选择保持不变。
- 验证：`cargo check --locked`、自动同步专项测试（4 项）和 `pnpm build` 通过，其中新增旧配置 90 天自动归一为 7 天的回归测试。
- 正式发布：使用唯一入口 `desktop-next/scripts/build-tauri-release.cmd` 生成内嵌正式前端的 Release，并覆盖根目录 `ozon-analytics-next.exe`。Release 与根目录文件均为 31,515,136 bytes，SHA-256 均为 `15B3B207787766F5EBC22B23210393A1E99B394A4D9071FF49963A275AB1AE9F`；根目录新版已启动且进程响应正常。
## 2026-09-18 Finance 逐笔应计 SKU 归属与已核算销量修复

- 原因：`/v1/finance/accrual/by-day` 的实际返回将数值型 SKU 放在 `item_fees.fees[].sku` 或 `posting.products[].sku`，而旧归一化仅按字符串读取 `posting.products`，造成全部费用被标记为店铺级未分摊，已结算销量也无法识别。
- 修复：Finance 归一化同时兼容新旧两种结构、字符串/数值标识符和 `accrual_id`；按正/负销售应计识别已交付/退货操作。月报打开时会在本地自动修复旧 `AccrualType0` 缓存，无需重新下载整月 Finance。
- 数据审计：默认本土店 2026-09 共 10,493 个唯一 Finance 应计 ID，重复 ID 为 0；其中可从原始响应恢复约 9,991 笔单 SKU 归属，销售结算操作约 2,597 笔。
- 验证：`cargo check --locked` 通过；Rust 166 项测试中 162 通过、4 项按设计忽略；`pnpm build` 通过；新增 Finance 新结构归一化与旧缓存修复测试通过。
- 发布：使用 `desktop-next/scripts/build-tauri-release.cmd` 构建正式版并覆盖根目录 EXE；大小 31,630,336 字节，SHA-256 `F84289C7F792E9366BE925E49C633104A4C518AC4CD5FBA3BDBEE056E7B0A767`。根目录程序已启动且进程响应正常；当前计算机界面工具未枚举出该原生窗口，未完成月报页面的截图级可见验收。
## 2026-09-18 启动自动同步店铺选择与月报首屏性能优化

- 数据同步页新增“启动时同步店铺”多选区，显示已选/全部店铺数；下次软件启动只遍历勾选店铺，手动“立即同步全部店铺”和定时同步保持原全店行为。
- 旧 `auto-sync.json` 没有选择字段时自动迁移为勾选当前全部店铺；保存时校验店铺 ID，并在开启启动同步时要求至少选择一家。
- 月度盈亏卡顿原因是首次修复 10,493 条旧 Finance 缓存时，每条 `UPDATE` 都单独自动提交。现已改为一次 SQLite 事务批量修复和单次缓存清理，避免长时间占用页面请求。
- 验证：`cargo check --locked` 通过；Rust 167 项测试中 163 通过、4 项按设计忽略；`pnpm build` 通过；新增旧设置自动选中全店的迁移测试。
- 发布：通过 `desktop-next/scripts/build-tauri-release.cmd` 生成正式版并覆盖根目录 EXE；大小 31,680,512 字节，SHA-256 `95B28D288F98DD95A294F6FB0DF986A531CF6857608A1F6F064C963DE90CD861`。根目录程序已启动且进程响应正常；当前计算机界面工具仍未枚举出该原生窗口，未完成勾选区的截图级可见验收。

## 2026-09-18 Finance 费用分类名称恢复

- 原因：上一轮旧缓存 SKU 归属修复在重建 Finance 明细时缺少应计类型字典，把原有 `Logistic`、`Acquiring`、`PackageCost` 等语义名称覆盖为 `AccrualTypeXX`。金额并未丢失，但分类器无法识别编号名称，导致物流、退货物流、仓储包装、收单支付、罚款及 Finance 广告被集中计入“其他 Finance 项目”。
- 修复：新增稳定的 Ozon 应计类型 ID 名称映射，并继续优先使用 `/v1/finance/accrual/types` 返回的实时字典；Finance 归一化和旧缓存修复统一使用该映射，未知类型仍保留编号以便审计。
- 迁移：月度盈亏首次打开时扫描包含 `AccrualType` 占位名称的旧明细，在单个 SQLite 事务中重新生成分类名称、清除月报缓存并记录修复版本；无需重新同步整月数据，也不会重复迁移。
- 验证：`cargo check --locked`、`pnpm build` 通过；Rust 全量测试 168 项中 164 通过、4 项按设计忽略，新增无 API 字典时仍能保持已知费用分类名称的回归测试。
- 发布：使用 `desktop-next/scripts/build-tauri-release.cmd` 构建并覆盖根目录 `ozon-analytics-next.exe`；Release 与根目录文件均为 31,682,048 字节，SHA-256 均为 `7008020DA398FB4FD5538E3053A11D7E9FD5F741F4C90D2E4B32258637235C1E`。

## 2026-09-18 启动同步改为显式开启

- 默认关闭“软件启动后同步”，让用户先配置启动店铺后再自行开启，手动同步与定时同步不受影响。
- 为兼容旧版本已经保存的自动同步配置，读取旧 `auto-sync.json` 时执行一次迁移：强制关闭启动同步并写入同意版本标记；用户随后在页面勾选并保存后，后续启动不会再次覆盖该选择。
- 新增回归测试覆盖默认关闭、旧配置一次性关闭及用户重新开启后保持开启。
- 验证：Rust 全量测试 165 passed / 0 failed / 4 ignored，`cargo check --locked`、`pnpm build` 通过。
- 发布：使用 `desktop-next/scripts/build-tauri-release.cmd` 构建并覆盖根目录 `ozon-analytics-next.exe`；Release 与根目录文件均为 31,681,536 字节，SHA-256 均为 `175B126D30BAB3381F2F61CD8E87B306EA96B1620A8A58FA48208115566CA907`。新版已启动且进程响应正常。

## 2026-09-18 自动同步设置保存竞态修复

- “保存设置”不再因全店同步运行而禁用；配置写入与同步任务相互独立，“立即同步”仍在任务运行期间防止重复点击。
- 修复页面每 3 秒轮询状态覆盖未保存表单的问题：有本地编辑时只刷新运行状态与结果，保留启动开关、定时开关、间隔和店铺选择，保存成功后再恢复完整状态刷新。
- 修复后台同步任务使用启动时旧快照回写 `auto-sync.json` 的竞态：每次写入进度和最终结果前在文件锁内合并最新配置，确保同步结束不会撤销用户刚保存的店铺选择。
- 启动时发现上次遗留的 `running` 状态且当前没有实际任务时，自动标记为“上次同步被中断”，解除按钮永久禁用。
- 验证：Rust 全量测试 167 passed / 0 failed / 4 ignored，新增配置竞态与中断状态恢复测试；`cargo check --locked`、`pnpm build` 通过。
- 发布：Release 与根目录 `ozon-analytics-next.exe` 均为 31,684,608 字节，SHA-256 均为 `C215406BCE4C94B2120B81BBC44ECC6A02DA1FB05F02E3394083DCD165D960D0`。

## 2026-09-18 手动一次性同步店铺选择

- 自动同步设置新增独立的“手动一次性同步店铺”列表，与“启动时同步店铺”分别持久化，修改任意一组后统一通过“保存设置”保存。
- 原“立即同步全部店铺”改为“立即同步已选店铺”，按钮显示当前手动选择数量；执行时将所选店铺 ID 传给后端，只遍历有效选中店铺，未选择时明确提示。
- 旧配置首次读取时手动同步列表默认选择全部已有店铺，保持升级前行为；用户保存子集后不再自动补回其他店铺。
- 后台同步进度写入会同时保护启动选择与手动选择，避免运行中的状态更新覆盖配置。
- 验证：Rust 全量测试 167 passed / 0 failed / 4 ignored，`cargo check --locked`、`pnpm build` 通过。
- 发布：Release 与根目录 `ozon-analytics-next.exe` 均为 31,692,800 字节，SHA-256 均为 `DB46A5E4FE4AA582D56363C34270B79BA6970F2D7C452C88A77C60D95C0F3E82`。

## 2026-09-18 约仓供应单货品与箱唛明细

- 约仓计划的活动供应单新增“货品 / 箱唛”按需查询；明细按“产品、SKU、货号、箱唛”展示，并附带供应数量，不在页面进入时批量加载，避免供应单较多时卡顿。
- 数据链路使用供应单详情取得 `supply_id` 与 `bundle_id`，通过 bundle 明细解析商品和货号，再以 cargo 明细中的 `bundle_id` 将 `cargo_id` 精确关联到 SKU；一个商品分布于多个箱唛时全部展示，尚未分配时明确显示“未设置箱唛”。
- 商品名称或货号缺失时仅回退当前店铺本地商品库；箱唛接口单独失败时保留已经读取的商品明细，并把具体失败原因显示在明细区，不伪造箱唛。
- 验证：`cargo check`、Rust 全量测试（167 passed / 0 failed / 4 ignored）与 `pnpm build` 通过。
- 发布：通过 `desktop-next/scripts/build-tauri-release.cmd` 生成正式版并覆盖根目录 EXE；Release 与根目录文件均为 31,855,616 字节，SHA-256 均为 `2ABC5C5FB6F79209182E6CE3109DD1ED99A87326113E39E2A3B662080FB78EBA`。
- 实机反馈修复：Ozon 的商品 `sku` 与 `cargo_id` 实际为 JSON 数值，初版字符串解析将其过滤，造成“暂未返回货品明细”。现统一保留字符串和数值标识符；箱唛优先改读 `/v1/cargoes/supplies/get` 返回的可打印 `barcode`，不存在条码时才回退 `cargo_id`，并保留旧 `/v1/cargoes/get` 兼容路径。
- 修正版验证与发布：Rust 全量测试 169 passed / 0 failed / 4 ignored（新增数值标识符及条码优先级测试），`cargo check` 与 `pnpm build` 通过；正式版与根目录 EXE 均为 31,857,152 字节，SHA-256 `B0140D81D524708DFBBC3C8BA49C713C4975CB27A64C58FC87E473685C7C847A`。
- 排版与缓存优化：货品/箱唛从页面底部长表改为紧凑弹窗，固定表头、内部滚动、箱唛标签化，并显示数据来源和读取时间。成功读取的数据按当前店铺数据库和供应单 ID 持久缓存，跨页面及软件重启复用；仅弹窗中的“重新读取 Ozon”绕过缓存并覆盖旧数据。
- 缓存版发布：Rust 全量测试 169 passed / 0 failed / 4 ignored，`cargo check` 与 `pnpm build` 通过；根目录正式版大小 31,886,336 字节，SHA-256 `042A2F7B9ACFB507E834875221AADFFC955698B8F9753B1D00FAB159FA393CDB`。
- 读取状态看板：后端按供应单维护真实任务状态，覆盖供应单详情、箱唛信息、箱唛与 SKU 关联、商品 bundle 明细和完成/失败；前端读取期间每 350ms 拉取一次状态，展示阶段说明、已完成/总步骤和进度条，缓存命中直接呈现完成态。
- 看板版发布：Rust 全量测试 169 passed / 0 failed / 4 ignored，`cargo check` 与 `pnpm build` 通过；根目录正式版大小 31,932,416 字节，SHA-256 `CE751A91A5F039825345351141F15AD52A182BD196986B3A4B9FC18C7657EA20`。

## 2026-09-18 箱唛明细导出与文件批量下载

- 货品/箱唛弹窗新增“导出箱唛 Excel”：按商品与箱唛展开为一唛一行，固定列为“物流箱唛、物流箱号、Ozon 货号、Ozon SKU、箱唛”；前两列留空并以黄色标记供物流商填写，货号、SKU 与箱唛强制写为文本，避免长数字、前导零或科学计数法破坏标识符。
- Excel 导出复用当前店铺按供应单保存的明细缓存，不重复请求 Ozon；即使某商品尚未分配箱唛，也保留一行空箱唛用于人工核查。
- 新增“下载全部箱唛 PDF”：从 `/v1/cargoes/supplies/get` 精确取得每个供应的 `cargo_id`，按供应调用 `/v1/cargoes-label/create`，轮询 `/v1/cargoes-label/get` 的当前 `file_url`，下载并校验 PDF 后放入独立文件夹并自动打开。未调用已经废弃的 `file_guid` 下载接口。
- 验证：`cargo check`、`pnpm build` 与 Rust 全量测试通过（170 passed / 0 failed / 4 ignored）；新增测试验证一个商品多个箱唛会展开多行，且 Ozon SKU/货号以文本保存。
- 发布：正式版已覆盖根目录 `ozon-analytics-next.exe`；文件大小 32,001,536 字节，SHA-256 `23D0B06A510202B4BE49CAA8599AFF78DE0A00E27B15954A6EBA988223FB85FA`。

## 2026-09-18 约仓计划工作区排版重构

- 将原来同页纵向堆叠的“库存集群计划、活动供应单、预约时段”拆分为“计划生成”和“供应单预约”两个工作区，通过顶部双标签切换；相关功能没有删除，只在对应流程中展示。
- 顶部新增计划候选数、活动供应单数和当前所选计划摘要；计划生成区保持搜索、目标天数、计划数量及采用操作，供应单预约区保持查询时段、货品/箱唛、预约及箱唛导出下载。
- 计划与供应单列表改为视口内固定高度、内部滚动、吸顶工具栏和吸顶表头；供应单与预约时段继续采用主列表加右侧操作栏，减少页面整体纵向长度。
- 统一增强卡片层级、间距、表格密度、输入框、按钮、选择态和悬停反馈，并补充窄屏单列响应式布局。
- 验证：`pnpm build` 通过。
- 发布：正式版已覆盖根目录 `ozon-analytics-next.exe`；大小 32,005,632 字节，SHA-256 `B8D02061462C1D84B9550E22EAF970281B8AF5E3C3240EAB4B2F210C81A8294D`。

## 2026-09-18 全局时间显示统一

- 新增公共时间格式化工具，对接口返回的时间统一显示“业务时区（默认 Ozon 欧洲店铺为 Europe/Moscow） + 中国 Asia/Shanghai 时间”；原始 UTC 仅用于后台计算和写回接口，不再直接作为用户界面时间。
- 已覆盖约仓、供应单、同步日志、同步状态、任务记录、库存更新时间、价格中心历史等高频显示入口；约仓仍优先使用供应单返回的实际 `timezone_info.iana_name`。
- 日期筛选、统计周期和 API 提交参数继续保持原有日期/UTC 语义，避免展示层转换影响数据查询。
- 验证：`pnpm build` 通过；正式版大小 32,005,632 字节，SHA-256 `2602042D23BE8F5F0E8116C539432FDB0E97A8DC5EC62087B1C848556E60840A`。
- 发布：根目录正式版大小 32,005,632 字节，SHA-256 `139EE02644761339082E149C89B53FA7BD5D3D1C8BCFB3DFA5BD1F55BCB82630`。

## 2026-09-18 约仓时段时区显示修复

- 原因：Ozon 预约接口返回 ISO 8601 UTC 时间（以 `Z` 结尾），旧界面直接显示原始值；Ozon Seller 后台按仓库所在地时区显示，造成同一预约看起来相差数小时，但实际预约值没有改变。
- 活动供应单当前时段和可预约时段现在均以供应单 `timezone_info.iana_name` 转换并标注“仓库当地时间”，同时附加 `Asia/Shanghai` 中国时间，杜绝把 UTC、仓库时间和电脑时间混为一谈。
- 预约确认框同时列出仓库当地时间和中国时间；提交仍使用 Ozon 返回的原始 UTC 起止值，不对写入参数做二次转换，避免时区重复偏移。
- 当 Ozon 未返回有效仓库时区时明确按 UTC 展示，不根据地址或电脑时区猜测。
- 验证：`pnpm build` 通过。
## 2026-09-20 约仓双工作台与正式启动器更新

- 约仓计划拆分为直送与越库两个独立工作流：直送仅允许莫斯科集群；越库支持多集群、多 SKU 数量、Ozon 越库发货点搜索、本地草稿以及 30/60 分钟时段监控与桌面提醒。
- 新增真实 Ozon 草稿链路：按集群创建 DIRECT/CROSSDOCK 草稿、读取草稿校验与目的仓、查询草稿可用时段，并在二次确认后从草稿创建正式供应单。远程写请求使用单次提交，响应不明确时不自动重试。
- 验证：TypeScript/Vite 正式构建通过；Rust `cargo check` 通过；供应单与箱唛回归测试 3/3 通过。
- 发布：使用强制入口 `desktop-next/scripts/build-tauri-release.cmd` 生成内嵌正式前端的 Release EXE，并覆盖根目录 `ozon-analytics-next.exe`。文件大小 32,265,728 字节，Release 与根目录 SHA-256 均为 `6029C7F747204383039D2AA8C42D4FCD06092C1AD785BF71CC4CF7539BEE53CE`。
- 启动核验：根目录 EXE 已启动，进程路径正确且处于 Responding 状态；当前自动化环境未枚举到该原生窗口的可访问性树，因此未代替用户执行真实 Ozon 草稿写入验收。

## 2026-09-20 约仓新品搜索与手工计划

- 约仓“计划生成”不再只查询 `inventory_stock` 集群库存快照：输入 SKU、货号或商品名称后，会同时检索当前店铺完整 `products` 商品库。
- 对尚未产生集群库存/销量记录的新品，系统按当前店铺已有配送集群生成“新品手工计划”候选行，库存、在途、已申请和日均销量保持为 0，由用户自行填写约仓数量并选择直送或越库流程。
- 空搜索仍保持原有库存建议计划，不把全量商品与所有集群做笛卡尔展开；关键词搜索支持回车触发，并以标签明确区分手工计划，避免误认为系统建议量。
- 已核对当前注册店铺实际名称为 `GHGTH`，目标商品 SKU `5246274984` 已存在于该店铺商品库；修复后不再要求该商品先出单或先拥有库存集群记录。
- 验证：`cargo check --locked`、TypeScript/Vite 正式构建及 Rust 全量测试通过（170 passed / 0 failed / 4 ignored）。
- 发布：已关闭占用旧文件的运行实例、覆盖并重新启动根目录正式版；新版大小 32,270,336 字节，SHA-256 `1917E460749A5266E964BF0C56281FB4648B5420168F877DA84A635A99E56824`。

## 2026-09-20 采购合同打印与 Word 自适应排版

- 修复打印样式中两套 `@media print` 规则互相覆盖的问题；取消固定定位、固定 210mm 纸张与页面边距叠加造成的二次缩放，改由 `@page` 统一控制 A4 纵向和页边距，正文宽度随可打印区域自动适配。
- 浏览器预览与打印共用明确的列宽比例、自动换行、表头重复和防止单行/签章区被分页拆开的规则；长产品名、银行名、地址和账号不再撑破页面。
- Word 商品明细改为全页固定布局表格，设置差异化列宽、单元格内边距、垂直/水平对齐和重复表头；甲乙方信息由全角空格拼接改为真正的无边框双列表格，避免不同字体、Word 版本和打印机下错位。
- 验证：前端正式构建、`cargo check --locked` 与 Rust 全量测试通过（171 passed / 0 failed / 5 ignored）；生成 DOCX 样例后使用文档解析器确认 A4 尺寸、页边距、商品表 5 列和签章表 2 列结构。当前工作区未提供内置 LibreOffice，规范渲染器无法生成 PNG，因此未使用本机 Office 替代该渲染门禁。
- 发布：正式版已覆盖并重新启动根目录 `ozon-analytics-next.exe`；大小 32,274,432 字节，SHA-256 `5E16172996E9C3BA9CE0A4FBA7A32801290EE3BE40924845785C841FF634959C`。

## 2026-09-20 约仓集群中文化

- 根据当前所有店铺库存数据库中的实际值，建立 Ozon 集群中文映射，覆盖阿拉木图、阿斯塔纳、白俄罗斯、沃罗涅日、远东地区、叶卡捷琳堡、喀山、加里宁格勒、克拉斯诺达尔、克拉斯诺亚尔斯克、马哈奇卡拉、莫斯科区域、涅温诺梅斯克、新西伯利亚、鄂木斯克、奥伦堡、彼尔姆、罗斯托夫、萨马拉、圣彼得堡区域、萨拉托夫、特维尔、秋明、乌法和雅罗斯拉夫尔。
- 约仓计划列表、顶部当前选择摘要及已选计划说明统一显示中文集群名；底层仍保留 Ozon 原始名称和 `macrolocal_cluster_id`，API 查询、保存草稿和正式约仓参数不变。
- 验证：TypeScript/Vite 正式构建通过。
- 发布：根目录正式版已覆盖并重新启动；大小 32,274,432 字节，SHA-256 `0D1294D1820F5409B5A25266313C9FF3F4A5F96ADDF687927B2EDB2D1E6657B7`。
# 2026-09-20：约仓计划支持 Ozon 多集群统一越库

- 修复越库计划虽然允许勾选多行、实际却按集群分别调用单集群草稿接口的问题。
- 越库创建改用 Ozon `/v1/draft/multi-cluster/create`，一次提交 `clusters_info`，最多 20 个集群；直送仍使用独立直送草稿。
- 发货点保留 Ozon 返回的 `warehouse_type`；多集群草稿校验后，统一携带各集群目的仓查询时段，并在最终约仓时一次提交全部 `selected_cluster_warehouses`。
- 界面行选择、顶部统计和底部摘要均以多选集合为准；“保存并加入”可连续加入不同集群，并显示集群数、商品行数及合计数量。
- 验证：`pnpm build` 通过；`cargo check --locked` 通过；`cargo test --locked` 171 项通过、5 项按设计忽略。
- 正式构建：使用 `desktop-next/scripts/build-tauri-release.cmd`；Release 与根目录 `ozon-analytics-next.exe` 均为 32,358,912 字节，SHA-256 `48CA1359336D49C5DB9A8054B2550CE027ABAEB31F2772E7CF92C340EC9E2895`。
- 已关闭旧进程、覆盖并启动根目录正式版。当前电脑控制接口未暴露原生窗口，无法自动执行页面点击验收；进程启动与正式二进制一致性已核对。

## 2026-09-20：多集群草稿发货点枚举修复

- 修复 `/v1/warehouse/fbo/list` 返回的默认仓库类型枚举 `0` 被错误转发给 `/v1/draft/multi-cluster/create`，造成 `DropOffWarehouse.WarehouseType` HTTP 400。
- 搜索结果与草稿创建请求均规范化为草稿接口要求的 `CROSS_DOCK`，旧页面状态即使仍持有 `0` 也不会继续发送无效枚举。
- 验证：`cargo check --locked`、`pnpm build` 与枚举回归测试通过。
- 使用正式构建脚本重新打包并覆盖根目录启动器；大小 32,357,376 字节，SHA-256 `07F186CFFAA89E2F062BDB3BFB9B950A2A70B995FE58F6780944D7EC057AA301`，正式进程已启动。

## 2026-09-20：新版多集群草稿目的仓及时段解析修复

- 按 `/v2/draft/create/info` 新结构解析 `clusters[].warehouses[].storage_warehouse.warehouse_id`，多集群草稿校验成功后可得到全部集群目的仓。
- 多集群查时段和最终创建供应单使用 `supply_type=MULTI_CLUSTER`，不再误传 `CROSSDOCK`。
- 递归解析 `/v2/draft/timeslot/info` 的 `result.drop_off_warehouse_timeslots.days[].timeslots[]`；非 `UNSPECIFIED` 的 `error_reason` 会直接显示。
- 草稿卡片增加“查询中 / 找到 N 个 / 当前范围暂无 / 查询失败”反馈，空结果不再表现为按钮无响应。
- 验证：`pnpm build`、`cargo check --locked` 通过；3 项供应草稿专项测试通过。
- 正式构建、覆盖和启动完成；大小 32,235,008 字节，SHA-256 `763EA72D480EDD3EEBED5A80D7DB5844EDC3E753B9724ACD52149064D07D9BD6`。

## 2026-09-20：按指定草稿或供应单监控时段

- 越库自动检查增加“监控对象”，可选择一份已校验草稿或一份活动越库供应单。
- 定时监控只请求所选对象，不再遍历全部草稿或全部供应单；切换对象会停止旧监控，需明确重新开启。
- 未选择对象时禁止开启监控并给出提示；选择项和 30/60 分钟间隔保存到本机工作流草稿设置。
- 新建单个多集群草稿后自动将其设为监控对象，但仍需用户手动开启定时监控。
- 验证：`pnpm build`、`cargo check --locked` 通过；Rust 174 项测试通过、5 项按设计忽略。
- 正式构建、覆盖和启动完成；大小 32,235,008 字节，SHA-256 `1C21F4D518EC6FEF9E670BC64A97ECE2035BC78E548CBE8574D6615F6A719466`。

## 2026-09-20：草稿无目的仓诊断与错误查询回退修复

- 修复已有草稿但无目的仓时，“统一查询”错误回退并显示检查旧供应单的问题；现在会停在当前草稿并明确说明无法查询时段。
- 草稿状态新增集群总数、可用目的仓数、无目的仓集群名单和 Ozon `error_reasons`，便于定位不可接收商品或无仓集群。
- 无目的仓时按钮改为“暂无目的仓”并禁用，避免产生“点击无反应”的误解。
- 验证：`pnpm build`、`cargo check --locked` 通过；4 项供应草稿专项测试通过。
- 正式构建、覆盖和启动完成；大小 32,232,960 字节，SHA-256 `4CB915E38E920502DC68EDD797C8F4339774DB55DB2DC6EC399E14CB1BE3CEB1`。

## 2026-09-20：多集群集中配送点类型与界面语义修复

- 根因修复：`/v1/draft/multi-cluster/create` 的实际集中配送点现在按接口要求发送 `warehouse_type=DELIVERY_POINT`，不再错误发送 `CROSS_DOCK`；仓库搜索接口返回的默认或旧枚举也不会被转发到草稿接口。
- 界面明确区分“集中配送地址（实际送货及预约地址）”与“平台分配仓（Ozon 内部转运路由）”；平台分配仓只是 Ozon 按集群计算的内部转运参数，不再以“目的仓”误导用户。
- 查询时段按钮与状态提示改为集中配送语义；草稿尚未获得内部路由时，会明确提示等待平台分配，而不会误认为还要选择另一个实际送货地址。
- 旧草稿的请求参数不可修改，需使用新版重新创建多集群越库草稿后再读取平台路由和预约时段。
- 验证：`pnpm build`、`cargo check --locked` 通过；4 项供应草稿专项测试通过。
- 正式构建、覆盖和启动完成；Release 与根目录启动器均为 32,232,960 字节，SHA-256 `A64B50DE530D796D6D8FA72746E5E417D8E5E782C14090406D87DA6EC6DFDB98`；进程 PID 21604，启动后处于 Responding 状态。

## 2026-09-20：草稿平台分配仓兼容解析修复

- 根因：Ozon 多集群草稿详情存在两种兼容结构。新版为 `macrolocal_cluster_id + storage_warehouse`，实际部分响应仍为 `cluster_id + supply_warehouse`；原解析器只继承前一种集群字段，导致响应明明包含仓库却显示“12 个集群、平台分配仓 0”。
- 仓库递归解析现在同时继承 `macrolocal_cluster_id` 与 `cluster_id`，并兼容嵌套 `storage_warehouse`、`supply_warehouse`；能够自动生成 `/v2/draft/timeslot/info` 所需的集群与仓库 ID，无需预先创建正式供应单。
- 验证：5 项供应草稿专项测试通过（包含两种仓库结构回归用例），`cargo check --locked` 与 `pnpm build` 通过。
- 正式构建、覆盖和启动完成；Release 与根目录启动器均为 32,232,960 字节，SHA-256 `537E7522F5351A178B1F89174C9BF6C6E8E5A4473716BD004401D4BE9AFCE714`；进程 PID 39084，启动后处于 Responding 状态。
## 2026-09-20 — Ozon 多集群统一发运自动路由兼容

- 修正 `/v2/draft/create/info` 的新版返回解析：当 `availability_status.state=FULL_AVAILABLE` 且 `storage_warehouse=null` 时，按 Ozon 统一发运协议识别为“平台自动分配”，并向时段及供应单接口传递 `storage_warehouse_id=0`，不再误报“平台分配仓 0 / 等待路由”。
- 真实只读 API 验证草稿 `129458966`：12 个集群均为 `FULL_AVAILABLE`；使用 `MULTI_CLUSTER` 与仓 ID `0` 成功返回 2026-09-23 至 2026-10-05 的可预约时段，证实无需先创建正式供应单。
- 界面将该状态显示为“统一发运（Ozon 自动分配）/ 无需指定内部目的仓”，并将统计项改为“可用路线”。正式程序可见验收中，新草稿显示 12 条可用路线并成功展示 132 个可预约时段。
- 验证：`cargo test --locked supply_draft_tests`（6 项通过）、`cargo check --locked`、`pnpm build` 均通过。`cargo fmt --check` 仍报告工作区既有文件 `ad_experiments.rs`、`freight_quotes.rs` 等历史格式差异，本次未批量格式化以免覆盖无关修改。
- 正式构建使用 `desktop-next/scripts/build-tauri-release.cmd`。Release 与根目录 `ozon-analytics-next.exe` 大小均为 `32233472` 字节，SHA-256 均为 `B68F5A5A161F9FC1DBB29564CBFAE2FF798C665DC4F31C85FAEEB3490C0A795D`；根目录程序已重启，内嵌页面地址为 `http://tauri.localhost/`。

## 2026-09-20 — 约仓产品体积与分集群配送成本测算

- 约仓集群计划返回商品成本资料中的长、宽、高，并按 `长 × 宽 × 高 ÷ 1000` 计算单件包装体积（L）；缺少任一尺寸时明确显示“缺尺寸”，不会按 0 体积计算。
- 约仓计划新增“莫斯科 → 各集群配送成本测算”：可选择 SKU、填写售价，并自动切换 `≤300 ₽` / `＞300 ₽` 基础配送费档位；每个集群分别保存两档官方路线费率。
- 越库成本按 `单件体积 × 约仓数量 × 路线费率（₽/L）` 计算，数量变化会即时影响总体积和本次越库费用；基础配送与越库合计均保留可审计的组成项。
- Ozon 当前基础物流采用体积、价格档和路线固定矩阵，界面不内置伪造的通用公式。官方矩阵未录入时显示“待录入”；`1.5 ₽/L` 仅能通过明确按钮作为兜底估算填入，并与仓库/分拣中心当前 `0 ₽/L` 处理费区分。
- 测算商品、售价与各 SKU/集群费率保存在本机，下次打开页面继续使用。
- 验证：`pnpm build`、`cargo check --locked` 通过；`cargo test --locked supply_draft_tests` 6 项通过。`cargo fmt --check` 仍报告仓库既有 `ad_experiments.rs`、`freight_quotes.rs` 等历史格式差异，本次未批量格式化无关文件。
- 正式构建使用 `desktop-next/scripts/build-tauri-release.cmd`。Release 与根目录 `ozon-analytics-next.exe` 大小均为 `32240640` 字节，SHA-256 均为 `41308322BE598F396B5152F46B63FEC72BA21066ADC0310B96EA2CE43FC4E814`。
- 已启动根目录正式版并完成可见验收：窗口标题为 `Ozon ERP`，页面地址为 `http://tauri.localhost/`；约仓计划真实显示产品选择、售价、包装尺寸、单件体积、两档基础配送费、越库费率、本次越库费和合计参考。当前示例 SKU 未维护尺寸，正确显示“商品成本中未填写 / 缺尺寸”。
## 2026-09-20：约仓计划接入 Ozon 官方基础配送费矩阵

- 从 Ozon Seller Education 官方 CDN 保存现行 `logistika-fbo-fbs-28082026` 费率工作簿，并新增可重复执行的 `scripts/generate_ozon_logistics_tariffs.py`，只提取“莫斯科、莫斯科州及远东地区”为发货集群的路线，生成 31 个目的集群、1333 条体积/售价档费率。
- 约仓计划不再要求运营人员手工填写基础配送费或越库 ₽/L；选择 SKU 后根据包装体积、售价档和目的集群自动命中 2026-08-28 官方路线矩阵，同时保留官方通用费率作为无法识别路线时的明确降级结果。
- 越库展示遵循官方计费结构：交货点处理费 + 到目的仓路线运输费；整批体积向上取整到整数升。仓库/分拣中心处理费为 0 ₽/L，PVZ 为 5 ₽/L，PPZ 为 4 ₽/L，Ozon 上门取件为 1.55 ₽/L。平台路线未返回时仅使用官方通用 1.5 ₽/L 作预估，并明确提示最终金额在创建供应申请时由 Ozon 锁定。
- 验证：`pnpm build` 通过；`cargo check --locked` 通过；`cargo test --locked` 通过（177 passed、5 ignored）；使用 `desktop-next/scripts/build-tauri-release.cmd` 完成正式构建。
- 发布：Release 与根目录 `ozon-analytics-next.exe` 均为 32,247,808 bytes，SHA-256 `8475F3596330EBF469F3D0901AAD3E47A740B703C2945F994BDB6341E360643D`。已启动根目录正式版并在 `tauri.localhost` 内嵌页面可见验收：SKU 5246274984、1.508 L、售价 299 ₽ 能自动显示多个集群的官方两档基础配送费，旧手工费率输入框已移除。

## 2026-09-20：约仓计划与供应单预约解耦、草稿恢复及多产品越库

- 供应单预约视图不再渲染计划生成工作区；计划生成仅在“计划生成”视图出现，减少重复操作和页面纵向长度。
- 计划选品改为本地购物车模型：跨搜索、跨集群保留已选产品与数量，避免重新搜索后选择丢失；一次越库草稿按集群聚合多个 SKU 明细，支持同一请求提交多产品。
...
# 2026-09-23：独立价格、Ozon 补贴与跨境利润监控

- 新增“价格与利润监控”独立页面，统一适配本土店和跨境店，读取 Seller API `/v5/product/info/prices` 中的前台营销价、卖家促销价与设置原价。
- 最终定价按“促销价与原价取较低有效值”计算；Ozon 补贴金额为最终定价减前台价，补贴比例为补贴金额除以最终定价，负补贴归零。
- `product_price_cache` 向后兼容增加 `marketing_price` 字段，保留接口原始 JSON；旧店铺数据库在首次打开模块时自动迁移。
- 修正前台价来源：新版 `/v5/product/info/prices` 已不再返回买家前台价，因此读取时额外调用 Premium Pro `/v1/product/prices/details`，以 `customer_price.amount` 作为唯一前台价、以 `price.amount` 作为促销价。接口无权限或未返回时保持空值并提示，禁止用卖家价回退冒充前台价。
- 跨境利润扫描优先处理成本、头程、包装尺寸、重量完整的商品。按当前店铺汇率将定价换算为人民币，并复用既有跨境运费阶梯。
- 单件利润口径：最终定价 - 采购成本 - 广告 10% - `(采购成本 + 贴单费 + 运费) × 3%` - 运费 - 平台佣金 - 物流佣金 2% - 贴单费 2 元。人民币售价大于 134 元时平台佣金 15%，否则 13%。
- 扫描结果写入店铺独立数据库缓存，支持全量刷新、选择商品刷新和可配置利润率预警线；资料不完整的商品显示缺失项，不参与错误利润预警。

## 2026-09-23：价格全量扫描响应性优化

- 全量及选中商品扫描改为每批 5 个 SKU 分次调用，批次间刷新已处理、成功、失败进度；可在当前批次结束后停止，已写入的缓存不回滚。
- 价格接口遇到 429 不再在单次调用内长时间等待重试，并设置 25 秒单请求超时。扫描发现限频、认证或权限错误时停止后续批次，避免连续无效请求。
- 验证：`pnpm build`、`cargo check --locked`、`cargo test --locked price_intelligence`（3 项通过）、正式 `build-tauri-release.cmd` 均成功。根目录唯一启动器 `Ozon ERP.exe` 已覆盖为 Release 产物（32,973,824 bytes；SHA-256 `B37ECED90A793DA907A2665C4E88DF264E535495E4382C0A026B7CDF5BB75BBD`），进程启动后 Responding。未执行带真实 Ozon 账号的全量扫描或页面可见验收。

## 2026-09-23：跨境人民币显示与首次价格同步加速

- 跨境店的前台价、促销价、设置原价、最终定价和 Ozon 补贴金额统一显示为人民币；原始价格为 RUB 时按当前店铺汇率换算，原始价格为 CNY 时不重复换算。利润计算同样按原始币种判定。
- 打开页面只从店铺本地数据库读取缓存；“扫描未缓存商品”仅请求未同步过的 SKU，已有缓存不会在每次启动或再次点击时重复请求。需要更新已有商品时使用“更新所选”。
- 首次同步改为每批最多 25 个 SKU 调用 Seller 批量价格接口和批量 Price Details 接口，正常情况下每批最多 2 次价格 API 请求，而非逐 SKU 两次。瞬时接口故障不会写入不完整缓存，可重试；Price Details 权限拒绝时仅缓存可读取的卖家价格，前台价继续留空。
- 验证：`pnpm build`、`cargo check --locked`、`cargo test --locked price_intelligence`（3 项通过）、正式 `build-tauri-release.cmd` 均通过。根目录唯一启动器 `Ozon ERP.exe` 已覆盖，大小 33,003,520 bytes，SHA-256 `A4DE55F5036CC4E1CBEBA5FB38879A20BACE91B3F6A00AC480EBFC31ECA0388A`，与 Release 产物一致；启动后进程 Responding。未使用真实账号执行批量 API 或完成页面可见验收。

## 2026-09-23：更新所选商品时的界面响应

- 价格刷新命令改为异步派发到后台阻塞线程，避免 Seller API 等待和 SQLite 写入占用界面事件循环；店铺状态在任务启动时固定为快照。
- “更新所选”每批最多 5 个 SKU，进度条在首批请求前绘制，并允许在当前批次结束后停止。未缓存商品的首次批量扫描仍按每批最多 25 个 SKU 执行。
- 每批只改写本批 SKU 的利润快照，不再重写整店所有商品的快照。
- 验证：`cargo check --locked`、`cargo test --locked price_intelligence`（3 项通过）、`pnpm build` 和正式 `build-tauri-release.cmd` 均通过。根目录唯一启动器 `Ozon ERP.exe` 已覆盖为 Release 产物（33,136,640 bytes，SHA-256 `9E97B736C0E77CCFFC6171509C205049EE1D4794A4F3CF9712160F5E30189AC8`），进程启动后 Responding。尚未用真实 Seller 账号完成“更新所选”的可见验收。

## 2026-09-23：跨境费用独立计算与本土口径隔离

- 根据用户审核后的公式，将跨境阶梯运费与头程拆开：人民币最终定价和有效重量齐全即可计算跨境运费；总运费仅在头程有值时为“跨境运费 + 头程”。缺头程或包装尺寸不再隐藏已可计算的跨境运费。
- 广告费、平台佣金、物流佣金按最终定价独立显示；货损仍需采购成本与总运费；完整利润仍需采购成本、总运费及货损。包装体积不参与现有按重量计费公式，不再作为利润计算门槛。
- 本土店不调用跨境运费阶梯，页面明确其物流费用应使用本土实际结算口径，不将跨境估算当作本土物流费。低利润预警与其他缺失项可同时显示。
- 新增回归测试覆盖“有售价和重量、缺头程时仍显示跨境运费”。`cargo check --locked`、`cargo test --locked price_intelligence`（4 项通过）、`pnpm build` 与正式 `build-tauri-release.cmd` 均通过。根目录 `Ozon ERP.exe` 已覆盖为 Release 产物（33,137,152 bytes，SHA-256 `33D3381E8B8FBD0D3CFCB38EA47DDD3E4C8B29A2CA6A82A8398DCDF7A6A9A2E8`），进程启动后 Responding。未用真实店铺进行页面可见验收。

## 2026-09-23：跨境运费即全部运费

- 根据用户明确确认的模型，跨境阶梯运费本身包含全部运费，价格利润监控中不再读取头程参与跨境总运费、货损、利润计算，也不再将头程视为缺失项。跨境货损为 `(采购成本 + 贴单费 + 跨境运费) × 3%`。
- 页面删除“跨境运费 + 头程”及头程展示，明确显示“跨境运费（全部运费）”。本土店仍保持独立的实际结算口径，不使用跨境阶梯。
- 以最终价 ¥95、成本 ¥22.50、重量 0.6 kg 验证：跨境运费 ¥41.57、货损 ¥1.9821、利润 ¥3.1979；即使无头程记录也可完整核算。`cargo check --locked`、`cargo test --locked price_intelligence`（4 项通过）、`pnpm build` 和正式 `build-tauri-release.cmd` 均成功。
- 根目录唯一启动器 `Ozon ERP.exe` 已覆盖为 Release 产物（33,136,640 bytes，SHA-256 `AABA3A91998840A88C2594240368487495F6A3675F478EC53BBAEBC873C0AC6A`），进程启动后 Responding。尚未完成真实店铺页面的可见验收。

## 2026-09-23：预警商品改价试算与按活动区间处理

- 价格与利润监控支持选择全部或部分低利润预警商品；按跨境店独立利润模型反推严格高于预警线的最低人民币售价，显示建议价和预计利润率。手动输入目标售价可逐项提交，自动模式按建议价批量处理，执行前均需确认。
- 执行前重新读取 Ozon 活动、活动商品及商品价格，核验商品 ID、币种和活动价格范围。目标在活动允许范围内时修改促销价；低于返回的活动最低价时取下限；高于活动最高价时逐个退出相关促销，再修改普通售价。活动列表分页不完整、冻结、价格区间或币种无法核对时拒绝自动执行。每步提交结果单独展示；Ozon 多接口调用不具备事务回滚能力。
- 不改变本土店物流及利润模型，本土店不开放该跨境利润反推入口。未对真实 Ozon 账号提交改价或退出促销。
- 验证：`cargo check --locked`、`cargo test --locked reprice_tests`（2 项通过）、`pnpm build` 与正式 `build-tauri-release.cmd` 均通过。根目录唯一启动器 `Ozon ERP.exe` 已覆盖，大小 33,140,224 bytes，SHA-256 `3B0C772B4492063236C8D47BBE95447932B045E60D6F5632580B18570A2FC7E4`，与 Release 产物一致；启动后进程 Responding。未完成价格页面的可见验收或真实 Ozon API 改价验收。

## 2026-09-23：改价试算缺少 Ozon 商品 ID 修复

- 修复商品表的空字符串 ID 遮蔽价格缓存有效 ID 的查询。打开改价试算时，仅对缺少有效 ID 的所选商品，从 Ozon 商品目录按 SKU/货号补查并缓存；匹配不唯一或接口失败时仍禁止自动改价，避免错改商品。
- 单品价格刷新也可使用价格缓存中的货号与商品 ID，避免产品表字段为空时误报缺少标识。执行改价仍会再次核对实时商品 ID、币种和促销活动。
- 验证：`cargo check --locked`、`cargo test --locked reprice_tests`（4 项通过）、`pnpm build` 和正式 `build-tauri-release.cmd` 均成功。根目录 `Ozon ERP.exe` 已覆盖为 Release 产物（33,198,592 bytes，SHA-256 `8B038872426264739B1CEB34338FD428536598367EA364BF7784F9FFC963ACF6`），启动后进程 Responding。未用真实店铺完成商品 ID 补查及改价可见验收，也未提交真实改价。

## 2026-09-23：改价过程界面响应优化
