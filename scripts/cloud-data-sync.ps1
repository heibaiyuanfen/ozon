...
try{
 if($Action-eq'Push'){
  ...
  git -C $RepositoryRoot add -- "cloud-data/devices/$slotName";git -C $RepositoryRoot commit -m "data($slotName): update encrypted database snapshot";git -C $RepositoryRoot pull --rebase origin main;git -C $RepositoryRoot push origin main
 } else {
  git -C $RepositoryRoot pull --ff-only origin main;if(!(Test-Path $package)){throw "云端没有数据库 $slotName 快照"};$m=Get-Content -Raw $manifest|ConvertFrom-Json;$actual=(Get-FileHash -Algorithm SHA256 $package).Hash;if($actual-ne$m.sha256){throw '云端数据包 SHA-256 校验失败'}
